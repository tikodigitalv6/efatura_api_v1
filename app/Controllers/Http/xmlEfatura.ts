...
export default class xmlEfatura {

	public async efaturaXmlDefaulth2(logoBase64,qrbase64){
		...
		try {
            ...
            console.log('efaturaXml ------>',error)
        }

    }

	public async eIhracatXmlDefaulth(logoBase64,qrbase64){
		try {
			let data = `<?xml version="1.0" encoding="UTF-8"?>
			<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
				xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2"
									...
									<tr align="left">
										<td id="notesTableTd">
											...
											<xsl:if test="cbc:AllowanceChargeReason!= ''">
													...
													<b><xsl:value-of select="cbc:AllowanceChargeReason" /></b> :&#160;
			...
			console.log('ihracat error',error);
			
		}
	}

		
	public async efaturaXmlDefaulth3(logoBase64,qrbase64){

		try {
			console.log('yeni xml çalıştı')
            const filePath = Env.get("XML_PATH") + 'default_efatura.xml';

            // Read the XML file
            var data = fs.readFileSync(filePath) as any ;
            
            data = mustache.render(data.toString(),{logoBase64,qrbase64})
            // Encode the data
	...
	public async eIhracatXmlDefaulth2(logoBase64,qrbase64){
		try {
			const filePath = Env.get("XML_PATH") + 'default_eihracat.xml';
